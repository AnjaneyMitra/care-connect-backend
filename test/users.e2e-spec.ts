...
describe('UsersController (e2e)', () => {
    ...
    beforeAll(async () => {
        try {
            ...
            // Get seeded users
            const parent = await prisma.users.findUnique({ where: { email: 'parent@example.com' } });
            const nanny = await prisma.users.findUnique({ where: { email: 'nanny@example.com' } });

            if (!parent || !nanny) {
                console.error('Seeded users not found. Please run "npx prisma db seed" first.');
