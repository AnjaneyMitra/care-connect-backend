...
export class BookingsService {
    ...
    async getActiveBookings(userId: string, role: 'parent' | 'nanny') {
        const whereClause = role === 'parent' ? { parent_id: userId } : { nanny_id: userId };
        return this.prisma.bookings.findMany({
            where: {
                ...whereClause,
                status: {
            ...
            include: {
                jobs: true,
                users_bookings_nanny_idTousers: role === 'parent' ? { select: { profiles: true } } : undefined,
                users_bookings_parent_idTousers: role === 'nanny' ? { select: { profiles: true } } : undefined,
            },
        });
    }
}
