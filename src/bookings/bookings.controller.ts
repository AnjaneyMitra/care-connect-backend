...
export class BookingsController {
    ...
    @Post()
    async createBooking(@Body() body: { jobId: string; nannyId: string }, @Request() req) {
        // Assuming the creator is the parent
        return this.bookingsService.createBooking(body.jobId, req.user.id, body.nannyId);
    }
    ...
    async getBooking(@Param('id') id: string, @Request() req) {
        ...
        if (booking.parent_id !== req.user.id && booking.nanny_id !== req.user.id && req.user.role !== 'admin') {
    ...
    async startBooking(@Param('id') id: string, @Request() req) {
        ...
        if (booking.nanny_id !== req.user.id) {
            throw new ForbiddenException('Only the assigned nanny can start the booking');
        }
    ...
    @Put(':id/cancel')
    async cancelBooking(@Param('id') id: string, @Body('reason') reason: string, @Request() req) {
        const booking = await this.bookingsService.getBookingById(id);
        if (booking.nanny_id !== req.user.id && booking.parent_id !== req.user.id) {
