import { Controller, Post, Body, UseGuards, Req, Get, Param, UseInterceptors, UploadedFile, BadRequestException, Delete } from '@nestjs/common';
...
export class VerificationController {
    ...
    async uploadDocuments(@Req() req, @Body() dto: UploadDocumentDto, @UploadedFile() file: Express.Multer.File) {
        if (!file) {
            throw new BadRequestException('File is required');
        }
        return this.verificationService.uploadDocuments(req.user.id, dto, file.path);
    }
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post(':id/reject')
    async rejectVerification(@Param('id') id: string, @Body() dto: RejectVerificationDto) {
