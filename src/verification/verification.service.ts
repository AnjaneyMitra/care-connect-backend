...
export class VerificationService {
    ...
    async uploadDocuments(userId: string, dto: UploadDocumentDto, filePath: string) {
    ...
    async resetVerification(userId: string) {
        ...
        const user = await this.prisma.users.findUnique({
            where: { id: userId },
            select: { identity_verification_status: true, verification_rejection_reason: true, role: true }
        });

        if (!user) throw new NotFoundException('User not found');
        if (user.role !== 'nanny') throw new ForbiddenException('Only nannies can withdraw verification');
